...
class TradeExecutor:
    """Executes trades on Deribit based on signals from the Signal Engine."""

    def __init__(self, deribit: DeribitClient, synth: SynthClient, user_id: str):
    ...
    def _get_max_open_positions(self) -> int:
        """Get configured max open positions for this user."""
        result = (
    ...
    def _log_event(self, trade_id: Optional[str], event_type: str, detail: dict):
        """Write an audit event to trade_events table."""
    ...
    async def execute_signal(self, signal: dict, budget: float) -> dict:
        """Take a generated signal and execute it on Deribit.

        1. Check max open positions limit
        2. Find the matching option instrument on Deribit
        ...
        currency = asset  # BTC, ETH, etc.

        # Step 1: Find the best matching instrument on Deribit
        instrument = await self.deribit.find_best_option(
            currency=currency,
            direction=direction,
        ...
        order_book = await self.deribit.get_order_book(instrument_name)
        best_ask = order_book.get("best_ask_price")
        best_bid = order_book.get("best_bid_price")
        mark_price = order_book.get("mark_price", 0)

        if not best_ask or best_ask <= 0:
            # No liquidity — use mark price
            limit_price = mark_price if mark_price > 0 else None
        else:
            # Place between mid and ask for better fill
        ...
        # Deribit options amount is in contracts (1 contract = 1 BTC for BTC options)
        # For small budgets, use minimum contract size
        min_amount = instrument.get("min_trade_amount", 0.1)
        amount = max(min_amount, min_amount)  # Start with minimum

        # Step 4: Place the order
        try:
            order_result = await self.deribit.buy(
        ...
        trade_data = {
            "user_id": self.user_id,
            "signal_id": None,  # Will be linked if signal was saved
            "asset": asset,
            "direction": direction,
        ...
        # Log event & push WebSocket update
    ...
    async def close_position(self, trade_id: str) -> dict:
        ...
        if trade.get("deribit_order_id"):
            try:
                order_state = await self.deribit.get_order_state(trade["deribit_order_id"])
                if order_state.get("order_state") == "open":
                    await self.deribit.cancel(trade["deribit_order_id"])
            except Exception:
                pass

        # Get current positions and close if filled
        positions = await self.deribit.get_positions(trade["asset"], "option")
        closed_price = None

        for pos in positions:
            if pos.get("direction") == "buy" and pos.get("size", 0) > 0:
    ...
    async def monitor_trade(
        ...
        Runs a loop that re-checks Synth data and P&L every `check_interval` seconds.
        """
        result = self.db.table("trades").select("*").eq("id", trade_id).execute()
        if not result.data:
            return {"status": "error", "reason": "Trade not found"}
        ...
        while True:
            try:
                # Check P&L from Deribit order book
                if trade.get("deribit_order_id"):
                    order_state = await self.deribit.get_order_state(trade["deribit_order_id"])
                    ...
                    # Check current mark price for P&L
                    if state == "filled" and order_state.get("instrument_name"):
                        order_book = await self.deribit.get_order_book(order_state["instrument_name"])
                        current_price = order_book.get("mark_price", 0)
                ...
                # Re-fetch Synth data for signal checks
                ...
                # Push periodic monitoring update
    ...
    async def auto_scan_loop(self, stop_event: asyncio.Event) -> None:
        ...
        while not stop_event.is_set():
            try:
                ...
                asset = settings.get("default_asset", "BTC")
                ...
                # Check if bot was disabled via settings
                ...
                if signal["action"] != "HOLD":
                    ...
                    if result["status"] == "executed" and result.get("trade_id"):
                        # Start monitoring this trade in parallel
                        asyncio.create_task(
                            self.monitor_trade(
            ...
            # Wait for next scan (check stop_event every second for responsiveness)
